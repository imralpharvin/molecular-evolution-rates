...
# Currently, median latitude is the only trait whose information is taken from BOLD.
...
#4.Renaming columns
colnames(traitData) <- c("species_name", "AdultBodyMass(g)", "AdultForearmLength(mm)", "BasalMetRate(mLO2hr)","LitterSize", "MaxLongevity(months)", "SexualMaturityAge(days)", "SocialGrpSize", "HabitatBreadth", "DietBreadth", "TrophicLevel", "ActivityCycle")
...
#11
traitFiltered <- traitFiltered[, c(1:3, 6:15)]
#12
missing <- traitFiltered[, apply(.SD, 1, function(x) all(is.na(x))), .SDcols = 4:13]
