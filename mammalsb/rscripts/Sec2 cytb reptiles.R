...
dfFilteredSingle <- dfcytB[!duplicated(species_name)]
...
dfTrait<- dfTrait[!is.na(species_name)]
...
dfTraits<- dfTraits[!is.na(species_name)]
...
dfPreCentroid <- merge(dfcytB, dfTraits, by = "species_name")[, 1:8]
# Dataframe reorganization and renaming.
setnames(dfPreCentroid, "accession_number.x", "accession_number")
setnames(dfPreCentroid, "sequence.x", "sequence")
