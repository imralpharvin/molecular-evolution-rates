...
dfLatitudeSpecies[, median_lat := median(abs_lat_num), keyby = accession_number]

#### TRAIT: LATITUDE RANGE ####
# Get maximum latitude for each bin
dfLatitudeSpecies[, max_lat := max(lat_num), keyby = accession_number]
# Get minimum latitude for each bin
dfLatitudeSpecies[, min_lat := min(lat_num), keyby = accession_number]
# Subtract maximum latitude and minimum latitude
dfLatitudeSpecies[, range_lat := max_lat - min_lat, keyby = accession_number]
...
dfFiltered <- dfFiltered[, .(accession_number, filtered_bin_size, recordID, order_name = order_label, family_name = family_label, genus_name = genus_label,
