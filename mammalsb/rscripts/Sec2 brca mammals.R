...
dfFilteredSingle <- dfBrca[!duplicated(species_name)][, .(accession_number, species_name)]
...
dfTraits$social_grpsize <- NULL
dfTraits$pop_grpsize <- NULL
dfTraits$neonate_svl_length<- NULL
dfTraits$growth_rate <- NULL
dfTraits$maturity_length <- NULL
dfTraits$forearm_length <- NULL

dfPreCentroid <- merge(dfBrca, dfTraits, by = "species_name")[, 1:3]
